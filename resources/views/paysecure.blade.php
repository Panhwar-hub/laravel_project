    ...
    <section class="breadcumb" style="background-image: url(assets/images/pg-banner.png);">
        <div class="container">
            <div class="breadcumb-con">
                <?php App\Helpers\Helper::inlineEditable('h2', ['class' => ''], 'Payment', 'CHECKOUTTXT1'); ?>
            </div>
        </div>
    </section>
                            ...
                            <table class="table order-summary-table shipping-table">
                                ...
                                <tbody>
                                    <tr class="checkout-item-label">
                                        <td colspan="2">
                                            <a id="checkout-toggle" href="#"
                                                onclick="$('#checkout-item-box').slideToggle(500);">1 Item in Cart <i
                                    ...
                                    <tr id="checkout-item-box">
                                        @if (!empty($itemsss))
                                            ...
                                            <tr class="checkout-item">
                                                <td class="checkout-product-image"><img src="{{ asset($value['image']) }}"
                                                        class="img-responsive lazyload" alt="Product Image"></td>
                                                <td colspan="3">
                                    ...
                                    <?php
                                        $grand_total = $total;
                                        if(isset($orders['delivery_fees'])){
                                            $grand_total = $orders['total_amount'];
                                    ...
                                    <tr class="order-total">
                                        <td>Total</td>
                                        <td class="get_total">${{ number_format($grand_total, 2) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
    ...
    <script>
        ...
        async function handleSubmit(e) {
            ...
            const { error } = await stripe.confirmPayment({
                elements,
                confirmParams: {
                    return_url: "{{route('order.submit')}}?amount={{ $amount }}&order_id={{ $order_id }}",
                },
            });
